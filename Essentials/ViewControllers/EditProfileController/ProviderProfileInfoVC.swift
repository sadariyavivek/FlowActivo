...
class ProviderProfileInfoVC: CoreViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        tblView.register(ProviderProfileInfoCell.nib, forCellReuseIdentifier: ProviderProfileInfoCell.identifier)
        countryApiCall(isLoader: false, type: "Countries", countryID: nil, stateID: nil)
    }
    
    private func fillArray() {
        if let cell = tblView.cellForRow(at: IndexPath(row: 0, section: 0)) as? ProviderProfileInfoCell {
    ...
    private func isValid() -> Bool {
        ...
        if let cell = tblView.cellForRow(at: IndexPath(row: 0, section: 0)) as? ProviderProfileInfoCell {
            if userData.businessname == nil || userData.businessname == "" {
                ...
                txtArray?.append(cell.txtPhone)
            } else if cell.btnMon.isSelected && userData.monday_start_time == nil || cell.btnMon.isSelected && userData.monday_start_time == "" {
                msg = "Please select monday start time."
                lblArry?.append(cell.lblMon)
            } else if cell.btnMon.isSelected && userData.monday_end_time == nil || cell.btnMon.isSelected && userData.monday_end_time == "" {
                msg = "Please select monday end time."
                lblArry?.append(cell.lblEndMon)
            } else if cell.btnTue.isSelected && userData.tuesday_start_time == nil || cell.btnTue.isSelected && userData.tuesday_start_time == "" {
                msg = "Please select tuesday start time."
                lblArry?.append(cell.lblTue)
            } else if cell.btnTue.isSelected && userData.tuesday_end_time == nil || cell.btnTue.isSelected && userData.tuesday_end_time == "" {
                msg = "Please select tuesday end time."
                lblArry?.append(cell.lblEndTue)
            } else if cell.btnWed.isSelected && userData.wednesday_start_time == nil || cell.btnWed.isSelected && userData.wednesday_start_time == "" {
                msg = "Please select wednesday start time."
                lblArry?.append(cell.lblWed)
            } else if cell.btnWed.isSelected && userData.wednesday_end_time == nil || cell.btnWed.isSelected && userData.wednesday_end_time == "" {
                msg = "Please select wednesday end time."
                lblArry?.append(cell.lblEndWed)
            } else if cell.btnThu.isSelected && userData.thursday_start_time == nil || cell.btnThu.isSelected && userData.thursday_start_time == "" {
                msg = "Please select thursday start time."
                lblArry?.append(cell.lblThur)
            } else if cell.btnThu.isSelected && userData.thursday_end_time == nil || cell.btnThu.isSelected && userData.thursday_end_time == "" {
                msg = "Please select thursday end time."
                lblArry?.append(cell.lblEndThur)
            } else if cell.btnFri.isSelected && userData.friday_start_time == nil || cell.btnFri.isSelected && userData.friday_start_time == "" {
                msg = "Please select friday start time."
                lblArry?.append(cell.lblFri)
            } else if cell.btnFri.isSelected && userData.friday_end_time == nil || cell.btnFri.isSelected && userData.friday_end_time == "" {
                msg = "Please select friday end time."
                lblArry?.append(cell.lblEndFri)
            } else if cell.btnSat.isSelected && userData.saturday_start_time == nil || cell.btnSat.isSelected && userData.saturday_start_time == "" {
                msg = "Please select saturday start time."
                lblArry?.append(cell.lblSat)
            } else if cell.btnSat.isSelected && userData.saturday_end_time == nil || cell.btnSat.isSelected && userData.saturday_end_time == "" {
                msg = "Please select saturday end time."
                lblArry?.append(cell.lblEndSat)
            } else if cell.btnSun.isSelected && userData.sunday_start_time == nil || cell.btnSun.isSelected && userData.sunday_start_time == "" {
                msg = "Please select sunday start time."
                lblArry?.append(cell.lblSun)
            } else if cell.btnSun.isSelected && userData.sunday_end_time == nil || cell.btnSun.isSelected && userData.sunday_end_time == "" {
...
extension ProviderProfileInfoVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: ProviderProfileInfoCell.identifier, for: indexPath) as! ProviderProfileInfoCell
        cell.user = userData
        var txtArray : [AppTextField]? = []
        var pickerArray : [CorePickerTextField]? = []
        ...
        cell.btnContinue.handleTap = {
            if self.isValid() {
                self.updateProfile(role: "Providers", type: "Update") {
                    let vcs = self.navigationController?.viewControllers ?? []
                    for vc in vcs {
                        if let sideMenuvc = vc as? SideMenuVC {
                            self.navigationController?.popToViewController(sideMenuvc, animated: true)
                        }
                    }
                }
            }
...
//MARK:- API CALLs
extension ProviderProfileInfoVC {
    ...
    private func updateProfile(role: String, type: String, completion: voidCompletion? = nil) {
